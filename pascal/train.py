...
from torch.utils.data import Dataset, DataLoader as PyTorchDataLoader, SubsetRandomSampler, SequentialSampler, RandomSampler # Added RandomSampler
...
class DatasetPASCAL(Dataset): # User-provided DatasetPASCAL
    ...
    def sample_episode(self, idx):
        ...
        if len(available_supports) < self.shot:
            ...
            if not support_pool: # If only query exists for this class (highly unlikely)
                 ...
                 logger.warning(f"Critically low unique support samples for class {class_sample}, query {query_name}. Sampling with replacement from non-query pool.")
                 if not support_pool: # If class only has one image (the query)
                    raise ValueError(f"Cannot sample {self.shot} support images for class {class_sample}, query {query_name}, as it's the only image.")
                 support_names = list(np.random.choice(support_pool, self.shot, replace=True))

            else:
                 support_names = list(np.random.choice(available_supports, self.shot, replace=len(available_supports) < self.shot))
        else:
    ...
    def build_img_metadata(self):
        def read_metadata(split, fold_id):
            # IMPORTANT: The user needs to ensure this path 'data/splits/pascal/...' exists
            # and contains the split files.
            # For robustness, one might pass this 'split_file_dir' as an argument.
            split_file_dir = os.path.join(os.path.dirname(self.img_path), "..", "splits", "pascal") # Heuristic path
            fold_n_metadata_path = os.path.join('./pascal/%s/fold%d.txt' % (split, fold_id))

            if not os.path.exists(fold_n_metadata_path):
                raise FileNotFoundError(f"PASCAL split file not found: {fold_n_metadata_path}. "
                                        f"Please ensure 'data/splits/pascal/' directory is correctly "
        ...
        logger.info('PASCAL Dataset: Total (%s) images for fold %d are : %d' % (self.split, self.fold, len(img_metadata)))
        return img_metadata
...
class PASCALAdapterDataset(Dataset):
    def __init__(self, pascal_dataset_instance, target_image_size_int, class_names_list):
        ...
        logger.info(f"PASCALAdapterDataset initialized. Wraps a PASCAL dataset with {len(self.pascal_dataset)} base items.")
        logger.info(f"Adapter will ensure output image/masks are ({target_image_size_int},{target_image_size_int}) and query GT is uint8.")
        ...
        try:
            ...
            for i in range(k_shot):
                ...
                if s_img.shape[1:] != self.target_image_size_tuple:
                    ...
                    s_img = TF.resize(s_img, list(self.target_image_size_tuple), interpolation=T.InterpolationMode.BILINEAR)
                if s_mask.shape != self.target_image_size_tuple:
                    ...
                    s_mask = TF.resize(s_mask.unsqueeze(0), list(self.target_image_size_tuple), interpolation=T.InterpolationMode.NEAREST).squeeze(0)
            ...
            if q_img_tensor.shape[1:] != self.target_image_size_tuple:
                ...
                q_img_tensor = TF.resize(q_img_tensor, list(self.target_image_size_tuple), interpolation=T.InterpolationMode.BILINEAR)
            if q_mask_tensor.shape != self.target_image_size_tuple:
                ...
                q_mask_tensor = TF.resize(q_mask_tensor.unsqueeze(0), list(self.target_image_size_tuple), interpolation=T.InterpolationMode.NEAREST).squeeze(0)
            ...
            logger.error(f"Error in PASCALAdapterDataset for index {idx} (Query: {raw_item.get('query_name', 'N/A') if 'raw_item' in locals() else 'N/A'}): {e}", exc_info=True)
...
DEFAULT_PASCAL_DATAPATH = './VOCdevkit/VOC2012' # NEW: Root for VOC2012, SegmentationClassAug, splits
DEFAULT_PRECOMPUTED_MASK_PATH = './sam_precomputed_masks_hybrid/pascal_voc2012_sam_masks' # NEW: Consider separate SAM masks for PASCAL
...
class FSSDataset(Dataset): # This class remains for now, but won't be used by main flow.
    def __init__(self, base_path, image_size=DEFAULT_IMG_SIZE, k_shot=DEFAULT_K_SHOT):
        ...
        try:
            ...
            logger.info(f"Initialized FSSDataset: {len(self.all_samples)} total samples across {len(self.classes)} valid classes from {base_path}")
        except Exception as e:
...
class FeatureExtractor(nn.Module):
    ...
    @torch.no_grad()
    def get_masked_features(self, feature_map, mask_tensor): # feature_map (B, D, Hf, Wf), mask_tensor (B, 1, Hm, Wm) or (B,Hm,Wm)
        ...
        elif mask_tensor.dim() == 3 and mask_tensor.shape[0] != feature_map.shape[0] : # (H, W) but passed as (B,H,W) from support items
             mask_tensor = mask_tensor.unsqueeze(1) # (B,1,H,W) assuming B matches FM
        elif mask_tensor.dim() == 3 and mask_tensor.shape[0] == feature_map.shape[0]: # (B,H,W) -> (B,1,H,W) (e.g. batch of support masks)
        ...
        if feature_map_expanded.shape[0] != mask_batch_size:
            logger.error(f"Batch size mismatch: Expanded FM ({feature_map_expanded.shape[0]}) vs Mask ({mask_batch_size}). Returning zeros.")
            return torch.zeros((mask_batch_size, num_features), device=self.device)
...
def _construct_graph_data_for_precomputation(support_set, query_set,
    ...
    # 1. Extract Individual Support Foreground Features
    ...
    for support_item in support_set: # support_item['mask'] is (H,W) float tensor
        ...
        s_mask = support_item['mask'] # (H,W) tensor, get_masked_features handles unsqueezing if needed
        if s_mask.sum() < 1e-6: 
            k_shot_support_features_list.append(torch.zeros(feature_extractor.feature_dim, device=feature_extractor.device))
            continue
        s_fm = feature_extractor.get_feature_map(s_img) 
        if s_fm is not None:
            # Pass s_mask as (H,W) and let get_masked_features handle batching/unsqueeze
            s_fg_feat = feature_extractor.get_masked_features(s_fm, s_mask.unsqueeze(0)) # Pass as (1,H,W)
    ...
    if not k_shot_support_features_list and len(support_set) > 0:
        logger.warning(f"{log_prefix} No support features extracted for {len(support_set)} items. Padding with zeros.")
        for _ in range(len(support_set)):
             k_shot_support_features_list.append(torch.zeros(feature_extractor.feature_dim, device=feature_extractor.device))

    while len(k_shot_support_features_list) < len(support_set):
        k_shot_support_features_list.append(torch.zeros(feature_extractor.feature_dim, device=feature_extractor.device))
    
    if not k_shot_support_features_list and len(support_set) > 0:
         logger.error(f"{log_prefix} Catastrophic failure in support feature extraction. Returning None.")
         return None
    
    if not support_set: 
        support_k_shot_features = torch.empty((0, feature_extractor.feature_dim), device=feature_extractor.device)
    else:
        support_k_shot_features = torch.stack(k_shot_support_features_list).to(feature_extractor.device)
    ...
    if ref_shape != (DEFAULT_IMG_SIZE, DEFAULT_IMG_SIZE): 
        logger.error(f"{log_prefix} Incorrect query GT mask shape {ref_shape}. Expected ({DEFAULT_IMG_SIZE}, {DEFAULT_IMG_SIZE})")
    ...
    try:
        ...
        mask_file_path = Path("./sam_precomputed_masks_hybrid/pascal_voc2012_sam_masks") / f"{query_idx}_sam_hybrid_masks.pkl"
    ...
    try:
        ...
        if x_base_features is None or x_base_features.shape[0] != num_nodes:
            logger.error(f"{log_prefix} Failed to extract base visual features for nodes. Got shape {x_base_features.shape if x_base_features is not None else 'None'}")
            return None
    except Exception as e:
    ...
    if num_nodes > 1:
        if connectivity == 'fully_connected':
            ...
            try:
                ...
                if k_actual > 0:
                    adj_sparse = kneighbors_graph(node_centroids_np, k_actual, mode='connectivity', include_self=False, n_jobs=1)
            ...
            for i in range(num_nodes):
                for j in range(i + 1, num_nodes): 
                    ...
                    if overlap_iou_val > overlap_thresh:
                        rows_overlap.extend([i, j]); cols_overlap.extend([j, i]) 
            if rows_overlap:
        ...
        if adj_matrix_for_edges_np is not None and adj_matrix_for_edges_np.shape[1] > 0:
            ...
            for src_idx, trg_idx in edge_index.t().tolist():
                mask1_node = masks_np_list_bool[src_idx]; mask2_node = masks_np_list_bool[trg_idx]
                current_edge_feats = []
                if edge_feature_mode == 'iou_dist_area': 
                    inter_mask_iou = calculate_iou(mask1_node, mask2_node)
                    c1 = np.array(node_centroids[src_idx]); c2 = np.array(node_centroids[trg_idx])
                    dist = np.linalg.norm(c1 - c2)
                    max_dist = np.sqrt(ref_shape[0]**2 + ref_shape[1]**2) 
                    norm_dist = dist / (max_dist + 1e-6)
                    area1 = mask1_node.sum(); area2 = mask2_node.sum()
    ...
    edge_attr = torch.stack(edge_attr_list) if edge_attr_list else torch.empty((0, expected_edge_dim), dtype=torch.float32) 
    
    if edge_index.numel() > 0 and edge_attr.shape[0] != edge_index.shape[1]:
        logger.error(f"{log_prefix} Mismatch: edge_attr rows ({edge_attr.shape[0]}) vs edge_index cols ({edge_index.shape[1]}). Clearing edges.")
        edge_index = torch.empty((2, 0), dtype=torch.long)
    ...
    try:
        data = Data(
            ...
            support_k_shot_features=support_k_shot_features.cpu(), 
        ...
        if not (data.x_base.shape[0] == data.x_geo.shape[0] == data.x_sam.shape[0] == data.y.shape[0] == data.num_nodes):
        ...
        if data.edge_index.numel() > 0 and data.edge_attr.shape[0] != data.edge_index.shape[1]:
            raise ValueError(f"Edge_attr row count ({data.edge_attr.shape[0]}) != edge_index column count ({data.edge_index.shape[1]}) for {query_class}/{query_idx}")
        if data.edge_attr.numel() > 0 and data.edge_attr.shape[1] != expected_edge_dim:
            raise ValueError(f"Edge_attr feature dim ({data.edge_attr.shape[1]}) != expected ({expected_edge_dim}) for {query_class}/{query_idx}")
        if data.support_k_shot_features.shape[1] != feature_extractor.feature_dim and data.support_k_shot_features.numel() > 0 : 
...
def precompute_graphs_entrypoint(dataset_adapter, indices, args, feature_extractor, device): # Takes adapter now
    ...
    for batch_items in pbar:
        ...
        class_graph_dir = Path(args.precomputed_graph_path) / query_class
...
class SupportCrossAttention(nn.Module):
    ...
    def forward(self, query_node_features, support_features_k_shot):
...
class SupportAwareMaskGNN(nn.Module):
    def __init__(self, base_node_visual_dim, geometric_dim, sam_meta_dim,
        ...
        logger.info(f"SupportAwareMaskGNN initialized. GNN Input Dim: {self.input_gnn_dim}, GNN Hidden: {gnn_hidden_dim}, GNN Layers: {num_gnn_layers}, Edge Dim: {edge_feature_dim}")

    def forward(self, data):
        x_base, x_geo, x_sam, support_k_shot_features, edge_index, edge_attr = \
            data.x_base, data.x_geo, data.x_sam, data.support_k_shot_features, \
...
def estimate_pos_weight(dataset_adapter, indices, precomputed_mask_path, args, device): # Takes adapter
    logger.info(f"Estimating pos_weight using {args.estimate_weight_subset:.1%} of {len(indices)} training samples (Greedy Oracle Labels)...")
    ...
    for batch_items in pbar_est:
        ...
        if query_class is None or query_idx is None or query_gt_mask_np is None:
            items_skipped_dataset += 1; continue
        ...
        try:
            ...
            mask_file_path = Path(precomputed_mask_path) / query_class / f"{query_idx}_sam_hybrid_masks.pkl"
    ...
    logger.info(f"Weight Estimation Done: Processed {nodes_processed} nodes from {processed_samples_count}/{len(subset_indices_for_est)} valid samples.")
...
def train_gnn(model, train_loader, optimizer, criterion, device,
    ...
    for batch_items in pbar: 
        ...
        graphs_in_batch = [] 
        original_data_for_iou_batch = [] 

        for item_idx, (_support_set, query_set) in enumerate(batch_items):
            query_class = query_set.get('class')
            query_idx = query_set.get('query_index')
            ...
            if graph_data is not None and graph_data.num_nodes > 0:
                if not hasattr(graph_data, 'x_base') or not hasattr(graph_data, 'y') or \
                   ...
                   graph_data.y.shape[0] != graph_data.num_nodes:
                    logger.warning(f"Epoch {args.current_epoch}, Batch {batch_num}, Item {query_class}/{query_idx}: Invalid graph data loaded. Skipping.")
                    skipped_items_load +=1
                    continue
                
                graphs_in_batch.append(graph_data)
                original_data_for_iou_batch.append({ 
        ...
        if not graphs_in_batch: continue 

        try:
            batch_graph_cpu = Batch.from_data_list(graphs_in_batch)
            batch_graph = batch_graph_cpu.to(device)
            del graphs_in_batch, batch_graph_cpu 
        except Exception as e:
            logger.error(f"Epoch {args.current_epoch}, Batch {batch_num}: Error creating/moving PyG Batch: {e}. Skipping.", exc_info=True)
            del graphs_in_batch 
            if 'original_data_for_iou_batch' in locals(): del original_data_for_iou_batch
            gc.collect(); torch.cuda.empty_cache() if "cuda" in device.type else None
            continue
            
        try:
            optimizer.zero_grad()
            if batch_graph.num_nodes == 0: continue 

            out_logits = model(batch_graph) 
            targets = batch_graph.y 

            if out_logits.shape[0] != targets.shape[0]:
                logger.error(f"Epoch {args.current_epoch}, Batch {batch_num}: Logits shape {out_logits.shape} != target shape {targets.shape}. Skipping.")
                continue
            
            loss = criterion(out_logits, targets)
            if torch.isnan(loss) or torch.isinf(loss):
                logger.warning(f"Epoch {args.current_epoch}, Batch {batch_num}: NaN/Inf loss ({loss.item()}). Skipping backward.")
            ...
            with torch.no_grad():
                ...
                for i in range(batch_graph.num_graphs):
                    start_node, end_node = ptr[i], ptr[i+1]
                    if end_node - start_node == 0: continue 

                    original_data = original_data_for_iou_batch[i]
                    ...
                    graph_pred_labels = pred_labels[start_node : end_node] 
                    
                    if len(node_masks_np_list) != (end_node - start_node):
                        logger.warning(f"Epoch {args.current_epoch}, Batch {batch_num}, Graph {i}: Mask count mismatch for IoU. Skipping this graph IoU.")
                        continue
                    ...
                    try:
                        ...
                        current_batch_ious.append(iou)
                    except Exception: 
                        current_batch_ious.append(0.0)
                epoch_train_ious.extend(current_batch_ious)

            total_loss += loss.item()
            processed_graphs += batch_graph.num_graphs
            processed_nodes += batch_graph.num_nodes
            
            pbar_postfix = {
                'loss': f"{loss.item():.4f}", 'avg_loss': f"{total_loss / batch_num:.4f}",
                'avg_train_iou': f"{np.mean(epoch_train_ious):.4f}" if epoch_train_ious else 0.0,
            }
            if args.clip_grad_norm > 0 and grad_norm_history: pbar_postfix['grad_norm'] = f"{grad_norm_history[-1]:.2f}"
            pbar.set_postfix(pbar_postfix)

        except Exception as e:
            logger.error(f"Epoch {args.current_epoch}, Batch {batch_num}: Error during training step: {e}. Skipping.", exc_info=True)
            if 'batch_graph' in locals(): del batch_graph
            if 'targets' in locals(): del targets
            if 'out_logits' in locals(): del out_logits
            if 'loss' in locals(): del loss
            optimizer.zero_grad() 
            gc.collect(); torch.cuda.empty_cache() if "cuda" in device.type else None
            continue
        
        del batch_graph, targets, out_logits, loss 
        if 'pred_probs' in locals(): del pred_probs
        if 'pred_labels' in locals(): del pred_labels
        del original_data_for_iou_batch

    avg_loss_epoch = total_loss / batch_num if batch_num > 0 else 0
    ...
    logger.info(f"Epoch {args.current_epoch} Train Finished. Avg Loss: {avg_loss_epoch:.4f}, Avg Train IoU: {avg_train_iou_epoch:.4f}, Avg Grad Norm: {avg_grad_norm:.4f}, Skipped Loads: {skipped_items_load}")
...
def evaluate_gnn(model, eval_loader, criterion, device,
    ...
    for batch_items in pbar: 
        ...
        try:
            ...
            logger.error(f"[Eval] Batch {batch_num}: Error creating/moving PyG Batch: {e}. Skipping.", exc_info=True)
            del graphs_in_batch
            if 'original_data_for_iou_batch' in locals(): del original_data_for_iou_batch
            gc.collect(); torch.cuda.empty_cache() if "cuda" in device.type else None
            continue
        ...
        try:
            ...
            for i in range(batch_graph.num_graphs):
                start_node, end_node = ptr[i], ptr[i+1]
                num_nodes_in_this_graph = end_node - start_node
                total_nodes_evaluated += num_nodes_in_this_graph

                if num_nodes_in_this_graph == 0: continue

                graph_node_pred_labels = pred_labels_nodes[start_node : end_node]
                ...
                if len(node_masks_np_list) != num_nodes_in_this_graph:
                    logger.warning(f"[Eval] Batch {batch_num}, Graph {i}: Mask count mismatch for IoU. Skipping IoU.")
                    continue
                ...
                try:
                    ...
                    logger.error(f"[Eval IoU Calc] Batch {batch_num}, Graph {i}: Error calculating final IoU: {e_iou}")
                    all_ious.append(0.0) 

            pbar_postfix = {'val_loss': f"{loss_val.item():.4f}" if loss_val is not None else "N/A"}
            if all_ious: pbar_postfix['avg_iou'] = f"{np.mean(all_ious):.4f}"
            if total_nodes_evaluated > 0: pbar_postfix['pos_preds%'] = f"{num_pos_preds_nodes / total_nodes_evaluated:.1%}"
            pbar.set_postfix(pbar_postfix)

        except Exception as e:
            logger.error(f"[Eval] Batch {batch_num}: Error during evaluation step: {e}. Skipping.", exc_info=True)
            if 'batch_graph' in locals(): del batch_graph
            if 'targets' in locals(): del targets
            if 'out_logits' in locals(): del out_logits
        ...
        del batch_graph, targets, out_logits, pred_probs, pred_labels_nodes
        if 'loss_val' in locals() and loss_val is not None: del loss_val
        del original_data_for_iou_batch
    ...
    pos_pred_rate_nodes = num_pos_preds_nodes / total_nodes_evaluated if total_nodes_evaluated > 0 else 0.0
    
    logger.info(f"Evaluation Finished. Avg Loss: {avg_loss_epoch:.4f}, Avg IoU: {avg_iou:.4f} (+/- {std_iou:.4f}), Node Pos Pred Rate: {pos_pred_rate_nodes:.2%}, Skipped Loads: {skipped_items_load}")
...
def main(args):
    ...
    if args.edge_feature_mode == 'iou_dist_area':
        ...
        logger.warning(f"Unknown edge_feature_mode '{args.edge_feature_mode}'. Defaulting to 'iou_only' with edge_feature_dim = 1.")
    ...
    if args.mode == 'precompute_graphs':
        logger.info("Mode: precompute_graphs. Initializing FeatureExtractor...")
        try:
            # Path checks for PASCAL
            if not os.path.isdir(args.pascal_datapath): raise FileNotFoundError(f"PASCAL datapath missing: {args.pascal_datapath}")
            if not os.path.isdir(args.precomputed_mask_path): raise FileNotFoundError(f"Precomputed SAM mask path invalid: {args.precomputed_mask_path}")
            feature_extractor = FeatureExtractor(device) 
        except Exception as e:
            logger.critical(f"Failed to initialize FeatureExtractor for precomputation: {e}", exc_info=True); return
    else: 
        logger.info(f"Mode: {args.mode}. FeatureExtractor will not be initialized.")
        if not os.path.isdir(args.pascal_datapath): raise FileNotFoundError(f"PASCAL datapath missing: {args.pascal_datapath}")
        if not os.path.isdir(args.precomputed_graph_path): raise FileNotFoundError(f"Precomputed graph path invalid: {args.precomputed_graph_path}")
        if args.use_loss_weighting and args.pos_weight_value is None: 
             if not os.path.isdir(args.precomputed_mask_path): raise FileNotFoundError(f"Precomputed SAM mask path (for weight est.) invalid: {args.precomputed_mask_path}")
    ...
    try:
        if args.mode == 'precompute_graphs':
            ...
            logger.info(f"Precomputation mode: Will use 'trn' split of PASCAL fold {args.pascal_fold}")
            precompute_pascal_dataset = DatasetPASCAL(
                datapath=args.pascal_datapath,
                fold=args.pascal_fold,
                transform=pascal_image_transform,
                split='trn', # Typically precompute on training data
                shot=args.k_shot,
                use_original_imgsize=args.pascal_use_original_imgsize # Should be False
            )
            dataset_for_precompute = PASCALAdapterDataset(
                precompute_pascal_dataset,
                args.img_size,
                PASCAL_CLASS_NAMES_LIST
            )
            all_dataset_indices = list(range(len(dataset_for_precompute)))
            logger.info(f"Precomputing graphs for {len(all_dataset_indices)} samples in 'trn' split of fold {args.pascal_fold}.")

            precomputation_start_time = time.time()
            precompute_graphs_entrypoint(
                dataset_adapter=dataset_for_precompute, 
            ...
            precompute_pascal_dataset = DatasetPASCAL(
                ...
                fold=args.pascal_fold,
                transform=pascal_image_transform,
                split='val', # Typically precompute on training data
                shot=args.k_shot,
                use_original_imgsize=args.pascal_use_original_imgsize # Should be False
            )
            dataset_for_precompute = PASCALAdapterDataset(
            ...
            precomputation_time = time.time() - precomputation_start_time
            logger.info(f"Graph precomputation finished in {time.strftime('%H:%M:%S', time.gmtime(precomputation_time))}.")
            logger.info(f"Graph precomputation finished in {time.strftime('%H:%M:%S', time.gmtime(precomputation_time))}.")
        ...
        logger.info(f"PASCAL datasets initialized. Train items: {len(train_dataset_adapter)}, Eval items: {len(eval_dataset_adapter)}")

    except Exception as e:
        logger.critical(f"Failed to initialize PASCAL dataset/adapter: {e}", exc_info=True); return
    ...
    try:
        ...
        train_loader = PyTorchDataLoader(train_dataset_adapter, batch_size=args.batch_size, sampler=train_sampler,
                                         collate_fn=collate_fn, num_workers=args.num_workers,
                                         pin_memory=pin_memory_flag, persistent_workers=persistent_workers_flag, drop_last=True)
        ...
        logger.critical(f"Failed to initialize DataLoaders for PASCAL: {e}", exc_info=True); return


    pos_weight_tensor = torch.tensor([1.0], device=device) 
    if args.use_loss_weighting:
        if args.pos_weight_value is not None: 
            pos_weight_tensor = torch.tensor([args.pos_weight_value if args.pos_weight_value > 0 else 1.0], device=device)
            ...
            try:
                ...
                if pos_weight_tensor.item() > 1000: pos_weight_tensor = torch.tensor([1000.0], device=device)
                elif pos_weight_tensor.item() <= 0: pos_weight_tensor = torch.tensor([1.0], device=device)
    ...
    try:
        ...
        logger.critical(f"Failed to initialize GNN model: {e}", exc_info=True); return
    
    try:
        optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    except Exception as e:
        logger.critical(f"Failed to initialize optimizer: {e}", exc_info=True); return
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            eval_loss, eval_iou = evaluate_gnn(
                model, eval_loader, criterion, device, args, 
            ...
            if eval_iou > best_eval_iou:
                ...
                except Exception as e: logger.error(f"Error saving best model: {e}")
            ...
            if args.save_interval > 0 and epoch % args.save_interval == 0:
                ...
                except Exception as e: logger.error(f"Error saving checkpoint epoch {epoch}: {e}")
        
        total_training_time = time.time() - start_train_time
        logger.info(f"======== Training Finished ({time.strftime('%H:%M:%S', time.gmtime(total_training_time))}) ========")
        logger.info(f"Best Validation IoU: {best_eval_iou:.4f}")
        try: torch.save(model.state_dict(), model_save_dir / "final_model.pth")
        except Exception as e: logger.error(f"Error saving final model: {e}")

        try:
            fig, axs = plt.subplots(3, 1, figsize=(10, 15), sharex=True)
            epochs_range = range(1, args.epochs + 1)
            axs[0].plot(epochs_range, train_losses, label='Training Loss', marker='.'); axs[0].plot(epochs_range, eval_losses, label='Validation Loss', marker='.')
            axs[0].set_ylabel('Loss'); axs[0].legend(); axs[0].grid(True, alpha=0.6); axs[0].set_title('Loss History')
            axs[1].plot(epochs_range, train_ious_epochs, label='Training IoU', marker='.'); axs[1].plot(epochs_range, eval_ious_epochs, label='Validation IoU', marker='.')
            axs[1].set_ylabel('Mean IoU'); axs[1].axhline(y=best_eval_iou, color='r', linestyle=':', label=f'Best Val IoU: {best_eval_iou:.4f}'); axs[1].legend(); axs[1].grid(True, alpha=0.6); axs[1].set_title('Mean IoU History'); axs[1].set_ylim(bottom=max(0, np.min(train_ious_epochs + eval_ious_epochs) - 0.05 if train_ious_epochs else 0))
            axs[2].plot(epochs_range, grad_norms_epochs, label='Avg Grad Norm (Train)', marker='.')
            axs[2].set_ylabel('Gradient Norm'); axs[2].set_xlabel('Epoch'); axs[2].legend(); axs[2].grid(True, alpha=0.6); axs[2].set_title('Avg Grad Norm History'); axs[2].set_yscale('log')
            fig.suptitle(f'Training History ({run_name})', fontsize=14); plt.tight_layout(rect=[0, 0.03, 1, 0.98])
            plt.savefig(model_save_dir / "training_history.png", dpi=150); plt.close(fig)
            logger.info(f"Training history plot saved to {model_save_dir / 'training_history.png'}")
        except Exception as e: logger.error(f"Error plotting training history: {e}", exc_info=True)

    elif args.mode == 'eval':
        logger.info(f"======== Starting Evaluation Mode ========")
        if not args.eval_model_path or not os.path.exists(args.eval_model_path):
            logger.error(f"Eval mode requires valid 'eval_model_path'. Path '{args.eval_model_path}' invalid."); return
        ...
        try:
            ...
            logger.error(f"Failed to load model from {args.eval_model_path}: {e}", exc_info=True); return
        
        logger.info(f"Starting evaluation on PASCAL fold {args.pascal_fold} validation set ({len(eval_dataset_adapter)} samples), threshold {args.eval_threshold}...")
        ...
        logger.info(f"--- Final Evaluation Results ---"); logger.info(f"Model: {args.eval_model_path}"); logger.info(f"Samples: {len(eval_dataset_adapter)}")
        logger.info(f"Time: {eval_time:.2f}s | Threshold: {args.eval_threshold:.2f} | Eval Loss: {eval_loss:.4f} | Mean IoU: {eval_iou:.4f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning("Execution interrupted by user (KeyboardInterrupt)."); sys.exit(0)
    except Exception as main_e:
        logger.critical(f"Unhandled exception in main: {main_e}", exc_info=True); sys.exit(1)
