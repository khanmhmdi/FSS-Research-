            ...
            try:
                ...
                if mode == "NORMAL_MODE":
                    ...
                    if not hasattr(args, 'visualized_once'):
                        ...
                        axs[0, 0].axis('off')
                        for k, s_item in enumerate(s_set):
                            if k >= num_to_show: break
                            s_img_np = (s_item['image'].cpu().numpy().transpose(1, 2, 0) * [0.229, 0.224, 0.225] + [
                                0.485, 0.456, 0.406])
                            axs[0, k + 1].imshow(np.clip(s_img_np, 0, 1));
                            axs[0, k + 1].set_title(f"Support {k + 1}");
                            axs[0, k + 1].axis('off')

                        axs[1, 0].imshow(q_set['gt_mask_np'], cmap='gray');
                        axs[1, 0].set_title("Query GT Mask");
                        axs[1, 0].axis('off')
                        for k, s_item in enumerate(s_set):
                            if k >= num_to_show: break
                            axs[1, k + 1].imshow(s_item['mask'].squeeze().cpu().numpy(), cmap='gray');
                ...
                # --- The rest of the evaluation proceeds normally ---
                targets = single_graph_batch.y
                loss_val = None
                if out_logits.shape[0] == targets.shape[0]:
